/**
 * Code from: https://stackoverflow.com/questions/30761082/mpandroidchart-round-edged-bar-chart
 */
...
class RoundedBarChartRenderer(
    ...
    override fun drawDataSet(c: Canvas, dataSet: IBarDataSet, index: Int) {
        ...
        if (mChart.isDrawBarShadowEnabled) {
            ...
            var x: Float
            var i = 0
            val count = Math.min(Math.ceil((dataSet.entryCount.toFloat() * phaseX).toDouble().toInt().toDouble()), dataSet.entryCount.toDouble())
            while (i < count) {
                ...
                if (!mViewPortHandler.isInBoundsRight(barShadowRectBuffer.left)) break
